...
import Lab5.grammar.firstLexer;
...
public class StartCompiler {
    public static void main(String[] args) {
        ...
        try {
            inp = CharStreams.fromFileName("src/Lab5/we.first");
        } catch (IOException e) {
...
//        CharStream inp = CharStreams.fromString("1\n2+3+4","wejście");
        ...
        STGroup group = new STGroupFile("src/Lab5/compiler/register.stg");

        EmitVisitor em = new EmitVisitor(group);
        ST res = em.visit(tree);
        System.out.println(res.render());
        try {
            var wr = new FileWriter("src/Lab5/wy2.asm");
            wr.write(res.render());
            wr.close();
        } catch (IOException e) {
            throw new RuntimeException(e);
